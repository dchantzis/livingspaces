...
if (!isset($_SESSION["SESSION"])) require ( "config.inc.php");
...
class TBDBase
{
	...
	if($connectionType == 0){
		@mysql_connect(SDB_HOST,SDB_COMMON_USER,SDB_COMMON_PASSWORD) or die('cannot connect '.SDB_HOST.SDB_COMMON_USER.SDB_COMMON_PASSWORD);
		@mysql_select_db(SDB_DATABASE) or die('cannot select database'.SDB_DATABASE);
	}//if
	else
	{
		switch($validateCredentials)
		{
			...
			case 'registereduser':	
				...
				@mysql_connect(SDB_HOST,SDB_COMMON_USER,SDB_COMMON_PASSWORD);
				...
				else
				{
					...
					@mysql_connect(SDB_HOST,SDB_REGISTERED_USER,SDB_REGISTERED_PASSWORD) or user_logout('ajax');
					@mysql_select_db(SDB_DATABASE) or user_logout('ajax');
				}//
				break;
